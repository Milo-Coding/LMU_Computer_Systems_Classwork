...
int main( int argc, char * argv[] ) {
   ...
   */
   if (fgets(copyLine, sizeof(copyLine), in) != NULL) {
   ...
   */
   for (int i = 0; i < copyCount; i++) {
      rewind(in); // Move the input file pointer back to the start of the input file
      ...
      fputc('\n', out); // Write a newline to the output file after each block
   }
  /*
